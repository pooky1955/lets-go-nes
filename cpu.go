...
type CPU struct {
	...
	Stack  [256]byte // Stack
	Status uint8     // Status register containing all flags
	C      bool      // Carry Flag
	Z      bool      // Zero flag
...
// SetFlag takes as input a flag and a value and updates the status register of the CPU
func (c *CPU) SetFlag(f Flag, value bool) {
	position := int(f)
	mask8 := ^uint8(1 << position)
	c.Status = (c.Status & mask8)
	if value {
		c.Status |= 1 << position
	}
}

// ReadFlag takes as input a flag and read it from the status register
func (c *CPU) ReadFlag(f Flag) bool {
	position := int(f)
	return c.Status&(1<<position) != 0
}
